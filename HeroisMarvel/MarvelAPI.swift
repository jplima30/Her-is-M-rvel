...
class MarvelAPI {
    ...
    class func loadHeros(name: String?, page: Int = 0, onComplete: @escaping (MarvelInfo?) -> Void) {
        ...
        AF.request(url).responseJSON { (response) in
            guard let data = response.data,
                  let marvelInfo = try? JSONDecoder().decode(MarvelInfo.self, from: data), marvelInfo.code == 200 else {
                onComplete(nil)
                return
            }
    ...
    private class func getcredentials() -> String {
        let timeStemp = String(Date().timeIntervalSince1970)
        let hash = MD5(timeStemp+privateKey+publicKey).lowercased()
        return "ts=\(timeStemp)1&apikey=\(publicKey)&hash=\(hash)"
    }
}
